...
var PopoverTrigger = React4.forwardRef(function PopoverTrigger2(componentProps, forwardedRef) {
  ...
  const preFocusGuardRef = React4.useRef(null);
...
function usePopupAutoResize(parameters) {
  ...
  const committedDimensionsRef = React17.useRef(null);
  const liveDimensionsRef = React17.useRef(null);
  const isInitialRenderRef = React17.useRef(true);
  const restoreAnchoringStylesRef = React17.useRef(NOOP);
  const onMeasureLayout = useStableCallback(onMeasureLayoutParam);
  const onMeasureLayoutComplete = useStableCallback(onMeasureLayoutCompleteParam);
  const anchoringStyles = React17.useMemo(() => {
...
var DirectionProvider = function DirectionProvider2(props) {
  ...
  return (0, import_jsx_runtime7.jsx)(DirectionContext.Provider, {
    value: contextValue,
...
function usePopupViewport(parameters) {
  ...
  const capturedNodeRef = React19.useRef(null);
  const [previousContentNode, setPreviousContentNode] = React19.useState(null);
  const [newTriggerOffset, setNewTriggerOffset] = React19.useState(null);
  const currentContainerRef = React19.useRef(null);
  const previousContainerRef = React19.useRef(null);
  const onAnimationsFinished = useAnimationsFinished(currentContainerRef, true, false);
  const cleanupFrame = useAnimationFrame();
  const [previousContentDimensions, setPreviousContentDimensions] = React19.useState(null);
  const [showStartingStyleAttribute, setShowStartingStyleAttribute] = React19.useState(false);
  useIsoLayoutEffect(() => {
  ...
  const lastHandledTriggerRef = React19.useRef(null);
  useIsoLayoutEffect(() => {
  ...
  if (!isTransitioning) {
    childrenToRender = (0, import_jsx_runtime8.jsx)("div", {
      "data-current": true,
      ref: currentContainerRef,
      children
    }, currentContentKey);
  } else {
    childrenToRender = (0, import_jsx_runtime8.jsxs)(React19.Fragment, {
      ...
      }, "previous"), (0, import_jsx_runtime8.jsx)("div", {
        "data-current": true,
        ref: currentContainerRef,
...
function usePopupContentKey(activeTriggerId, payload) {
  const [contentKey, setContentKey] = React19.useState(0);
  const previousActiveTriggerIdRef = React19.useRef(activeTriggerId);
  const previousPayloadRef = React19.useRef(payload);
  const pendingPayloadUpdateRef = React19.useRef(false);
  useIsoLayoutEffect(() => {
...
var PopoverViewport = React20.forwardRef(function PopoverViewport2(componentProps, forwardedRef) {
  const {
    render,
    className,
    children,
